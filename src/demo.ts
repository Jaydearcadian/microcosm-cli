...
import { createState, type LoopRun, type Scenario } from "./state.js";

const rail = (run: LoopRun) => {
...
const reasons = (run: LoopRun) =>
  run.policy.evaluation.reasons.length > 0 ? run.policy.evaluation.reasons.join("; ") : "all envelope checks passed";

const targetLabel = (target: LoopRun["payment"]["target"]) =>
  target.kind === "recipient" ? `recipient:${target.recipientId}` : `venue:${target.venueId}`;

export const createDemoRun = (scenario: Scenario): LoopRun => runOnce(createState(), scenario);

export const renderDemo = (run: LoopRun): string => {
