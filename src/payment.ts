...
const requestForScenario = (scenario: Scenario, runCount: number): EvaluationRequest => {
  if (scenario === "denied") return deniedRequest;
  if (scenario === "mixed") return runCount % 2 === 0 ? allowedRequest : deniedRequest;
  return allowedRequest;
};

export const createPayment = (state: ConsoleState, scenario = state.scenario): ProgrammablePayment => {
...
export const paymentToRequest = (payment: ProgrammablePayment): EvaluationRequest => ({
  version: "request.v1",
  id: payment.id,
