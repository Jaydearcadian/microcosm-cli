...
const rpcCall = async (config, method, params = []) => {
...
const parseHexQuantity = (value) => Number(BigInt(value));
const encodeBalanceOf = (address) => `0x${BALANCE_OF_SELECTOR}${address.slice(2).toLowerCase().padStart(64, "0")}`;
...
const readErc20Balance = async (config, token, address) => {
