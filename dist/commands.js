...
import { createState, latest } from "./state.js";
import { fail, json, ok, table } from "./output.js";
const boxes = ["payment", "policy", "proof", "settlement", "arc", "loop", "adapter"];
...
const scenarioFromArgs = (args, state) => {
...
const printable = (value, useJson) => useJson ? json(value) : `${JSON.stringify(value, null, 2)}\n`;
const requirePayment = (state) => latest(state.payments) ?? createPayment(state);
const requirePolicy = (state) => latest(state.policies) ?? evaluatePaymentPolicy(state, requirePayment(state));
const requireProof = (state) => latest(state.proofs) ?? composePaymentProof(state, requirePayment(state), requirePolicy(state));
export const handleCommand = async (args, state = createState()) => {
    const safetyErrors = assertSafeArgs(args);
    if (safetyErrors.length > 0)
        return fail(`error: ${safetyErrors.join("; ")}\n`, 2);
    const { group, subcommand, rest } = args;
    if (!group || group === "help")
        return ok(helpText());
    if (boxes.includes(group) && !subcommand)
        return ok(boxHelp(group));
    try {
        if (group === "status") {
            const status = {
                network: "arc-testnet",
        ...
        if (group === "history")
            return ok(printable(state.runs, args.json));
        if (group === "reset") {
        ...
        if (group === "scenario") {
            ...
            if (!value)
                return ok(`scenario: ${state.scenario}\n`);
            if (value !== "allowed" && value !== "denied" && value !== "mixed")
                return fail("error: scenario must be allowed, denied, or mixed\n", 2);
        ...
        if (group === "payment") {
            ...
            if (subcommand === "latest")
                return ok(printable(latest(state.payments) ?? null, args.json));
            if (subcommand === "list")
                return ok(printable(state.payments, args.json));
        ...
        if (group === "policy") {
            ...
            if (subcommand === "latest")
                return ok(printable(latest(state.policies) ?? null, args.json));
        ...
        if (group === "proof") {
            ...
            if (subcommand === "latest")
                return ok(printable(latest(state.proofs) ?? null, args.json));
            if (subcommand === "list")
                return ok(printable(state.proofs, args.json));
            if (subcommand === "get") {
                const id = rest[0];
        ...
        if (group === "settlement") {
            ...
            if (subcommand === "latest" || subcommand === "inspect")
                return ok(printable(latest(state.settlements) ?? null, args.json));
        }
        if (group === "loop") {
            if (subcommand === "once")
                return ok(printable(runOnce(state, scenarioFromArgs(args, state)), args.json));
            if (subcommand === "run") {
                const iterations = Number(args.flags.iterations ?? "0");
                if (!Number.isInteger(iterations) || iterations < 1)
                    return fail("error: loop run requires --iterations <n>\n", 2);
                return ok(printable(runLoop(state, iterations, scenarioFromArgs(args, state)), args.json));
            }
            if (subcommand === "status")
                return ok(printable({ runs: state.runs.length, latestRun: latest(state.runs)?.id ?? null }, args.json));
        }
        if (group === "adapter") {
            if (subcommand === "list" || subcommand === "status")
                return ok(printable(signerAdapters(), args.json));
        }
        if (group === "demo") {
        ...
        if (group === "arc") {
            ...
            if (subcommand === "rpc-info")
                return ok(printable({ network: config.network, rpcUrl: config.rpcUrl.replace(/\/\/.*@/, "//<redacted>@") }, args.json));
            if (subcommand === "status")
                return ok(printable(await getArcStatus(config), args.json));
            if (subcommand === "balance") {
                const address = rest[0];
                if (!address || !address.startsWith("0x"))
                    return fail("error: arc balance requires an address\n", 2);
                return ok(printable(await getArcBalance(config, address), args.json));
            }
        }
        return fail(`error: unknown command. Run ${group && boxes.includes(group) ? group : "help"} for options.\n`, 1);
    }
    catch (error) {
